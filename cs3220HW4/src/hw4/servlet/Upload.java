            ...
            if (formItems != null && formItems.size() > 0) {
                ...
                for (FileItem item : formItems) {
                    ...
                    Integer parentid = 1;
           			...
           			Integer accId = 3;
           			//Integer.valueOf( request.getParameter( "aid" ));
           			
   				 	
                    File1 entry = new File1(idSeed++, fileName, fileType, size, ft.format(date), parentid, false, accId);
                    
                    List<File1> entries = (List<File1>) getServletContext().getAttribute("entries" );
            	    entries.add( entry );
            	    
            	    if(entry.getParentId() == 1){
        	        	response.sendRedirect( "FileManager?accId=" + entry.getAccId() );
        	        }
        	        else{
